...
function PupilDetail() {
  ...
  const { data: pupil, isLoading } = useQuery({
    ...
    queryFn: () => api.pupils.get(pupilId),
  });
  const { data: guardians = [] } = useQuery({
    queryKey: ["pupil-guardians", pupilId],
    queryFn: () => api.guardians.byPupil(pupilId),
  });
  const { data: invoices = [] } = useQuery({
    queryKey: ["pupil-invoices", pupilId],
    queryFn: () => api.fees.invoices.list({ pupilId }),
  });
  const to = new Date().toISOString().slice(0, 10);
  const from = new Date(Date.now() - 90 * 864e5).toISOString().slice(0, 10);
  const { data: attendance = [] } = useQuery({
    queryKey: ["pupil-attendance", pupilId],
    queryFn: () => api.attendance.byPupil(pupilId, from, to),
  });
  const { data: health = [] } = useQuery({
    queryKey: ["pupil-health", pupilId],
    queryFn: () => api.health.list(pupilId),
  });
  const { data: discipline = [] } = useQuery({
    queryKey: ["pupil-discipline", pupilId],
    queryFn: () => api.discipline.list(pupilId),
  });
  ...
  if (!pupil)
    return (
      <div className="p-6">
        <EmptyState message="Pupil not found." />
      </div>
    );
  ...
  const attendancePct = attendance.length
    ...
    : null;
  const classLabel = pupil.schoolClass
    ? `${pupil.schoolClass.name}${pupil.schoolClass.stream ? " " + pupil.schoolClass.stream : ""}`
    : "Unassigned";
    ...
    <>
      <PageHeader
        ...
        actions={
          <Button variant="outline" onClick={() => navigate({ to: "/pupils" })}>
            <ArrowLeft className="h-4 w-4 mr-1" /> Back to pupils
          </Button>
        }
      />
      <div className="p-6 space-y-6">
        <ProfileHeader
          ...
          badges={
            <>
              <Badge variant={pupil.status === "ACTIVE" ? "success" : "secondary"}>
                {pupil.status}
              </Badge>
                          ...
                          <TableRow key={i.id}>
                            ...
                            <TableCell>
                              {bal > 0 && (
                                <PayOnlineDialog
                                  invoice={i}
                                  defaultPhone={guardians[0]?.phone}
                                  initiate={(data) => api.fees.invoices.payOnline(i.id, data)}
                                  checkStatus={(reference) =>
                                    api.fees.gatewayTransactionStatus(reference)
                                  }
                                  onSuccess={() =>
                                    qc.invalidateQueries({ queryKey: ["pupil-invoices", pupilId] })
                                  }
                                />
                              )}
                            </TableCell>
                    ...
                    <TableBody>
                      {discipline.map((d) => (
                        <TableRow key={d.id}>
                          ...
                          <TableCell>
                            <Badge variant="destructive">{d.kind}</Badge>
                          </TableCell>
