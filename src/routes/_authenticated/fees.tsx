...
function Fees() {
  const { data: pending = [] } = useQuery({
    ...
    queryFn: () => api.fees.payments.pending(),
  });
  return (
    <>
      <PageHeader
        title="Fees & payments"
        description="Manage invoices, record payments, track balances"
      />
      <div className="p-6">
    ...
    onSuccess: (created) => {
      ...
      toast.success(
        created.length === 0
          ...
          : `Charged K35 administrative fee on ${created.length} overdue invoice${created.length !== 1 ? "s" : ""}`,
      );
    },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  return (
    <div className="space-y-3 pt-4">
      ...
      <div className="flex justify-end gap-2">
        <Button
          variant="outline"
          onClick={() => applyLateFees.mutate()}
          disabled={applyLateFees.isPending}
          title="Charges a K35 administrative fee on invoices past their due date that haven't been fully paid"
        >
          ...
          <DialogContent>
            ...
            <DialogFooter>
              <Button
                ...
                disabled={!f.pupilId || !f.total || create.isPending}
              >
              ...
              pagedInvoices.map((i) => (
                <TableRow key={i.id}>
                  ...
                  <TableCell className="flex items-center gap-1">
                    <Button size="icon" variant="ghost" onClick={() => setPrintId(i.id)}>
                    ...
                    {canReverse && Number(i.paid) === 0 && (
                      <Button
                        size="icon"
                        variant="ghost"
                        title="Delete invoice (only available while unpaid)"
                        onClick={() => {
                          if (confirm(`Delete invoice ${i.invoiceNo}? This cannot be undone.`))
...
function InvoicePrint({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  const { data: inv } = useQuery({
    ...
    queryFn: () => api.fees.invoices.get(id),
  });
  const { data: payments = [] } = useQuery({
    queryKey: ["invoice-pays", id],
    queryFn: () => api.fees.payments.list({ invoiceId: id }),
  });
  if (!inv) return null;
        ...
        <div className="grid grid-cols-2 gap-8 text-sm mb-8">
          <div>
            ...
            <p className="text-gray-600">
              {inv.pupil?.schoolClass?.name ?? ""} {inv.pupil?.schoolClass?.stream ?? ""}
            </p>
          </div>
          <div className="text-right">
            ...
            {inv.dueDate && (
              <p>
                <span className="text-gray-500">Due:</span> {inv.dueDate}
              </p>
            )}
            <p
              className="mt-2 inline-block px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider"
              style={{
                background:
                  inv.status === "PAID"
                    ? "#dcfce7"
        ...
        <table className="w-full text-sm border-collapse mb-8">
          ...
          <tbody>
            <tr className="border-b border-gray-200">
              <td className="py-3">{inv.description || "School fees"}</td>
              <td className="py-3 text-right">{money(inv.total)}</td>
            </tr>
          </tbody>
          <tfoot>
            <tr>
              <td className="pt-4 text-right text-gray-500">Subtotal</td>
              <td className="pt-4 text-right">{money(inv.total)}</td>
            </tr>
            <tr>
              <td className="text-right text-gray-500">Paid</td>
              <td className="text-right">{money(inv.paid)}</td>
            </tr>
            <tr className="text-lg font-bold">
              <td className="pt-2 text-right">Balance due</td>
              <td className="pt-2 text-right">{money(balance)}</td>
            </tr>
          </tfoot>
        </table>
        {payments.length > 0 && (
...
function PaymentsTab() {
  ...
  const filteredInvoices = openInvoices.filter((i) => {
    const s = invoiceSearch.toLowerCase().trim();
    return (
      !s ||
      i.pupil?.fullName?.toLowerCase().includes(s) ||
      i.pupil?.admissionNo?.toLowerCase().includes(s) ||
      i.invoiceNo?.toLowerCase().includes(s)
    );
  });
  const {
    pageItems: pagedPayments,
    page,
    setPage,
    totalPages,
    pageSize,
  ...
  const update = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["payments"] });
      qc.invalidateQueries({ queryKey: ["invoices"] });
      toast.success("Payment corrected — invoice balance updated");
      setEditing(null);
    },
    onError: (e: any) => toast.error(e.message),
  });
  const remove = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["payments"] });
      qc.invalidateQueries({ queryKey: ["invoices"] });
      toast.success("Payment removed — invoice balance updated");
    },
    onError: (e: any) => toast.error(e.message),
  });
  const [f, setF] = useState({ invoiceId: "", amount: 0, method: "CASH", reference: "" });
  const selectedInvoice = openInvoices.find((i) => i.id === f.invoiceId);
  return (
    <div className="space-y-3 pt-4">
      {/* Filters */}
      <div className="flex flex-wrap items-end gap-3">
          ...
          <DialogContent>
            ...
            <div className="space-y-3">
              <div>
                <Label>Invoice</Label>
                <Input
                  placeholder="Search pupil name, admission no, or invoice no…"
                  value={invoiceSearch}
                  ...
                  <Table>
                    <TableBody>
                      {filteredInvoices.length === 0 ? (
                        <TableRow>
                          <TableCell className="text-center text-muted-foreground py-4 text-sm">
                        ...
                        filteredInvoices.slice(0, 50).map((i) => (
                          <TableRow
                            key={i.id}
                            className={`cursor-pointer ${f.invoiceId === i.id ? "bg-primary/10" : ""}`}
                            onClick={() => setF({ ...f, invoiceId: i.id })}
                          >
                            ...
                            <TableCell className="py-1.5 text-xs text-muted-foreground text-right">
                              {money(Number(i.total) - Number(i.paid))} due
                            </TableCell>
                          </TableRow>
                        ))
                      )}
                ...
                {selectedInvoice && (
                  <p className="text-xs text-muted-foreground mt-1">
                    Selected:{" "}
                    <span className="font-medium text-foreground">{selectedInvoice.invoiceNo}</span>{" "}
                    — {selectedInvoice.pupil?.fullName}
                  </p>
                )}
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Select value={f.method} onValueChange={(v) => setF({ ...f, method: v })}>
                    ...
                    <SelectContent>
                      {["CASH", "BANK", "MOBILE_MONEY", "CARD", "CHEQUE", "ONLINE"].map((m) => (
            ...
            <DialogFooter>
              <Button
                onClick={() => create.mutate(f)}
                disabled={!f.invoiceId || !f.amount || create.isPending}
              >
                {create.isPending ? "Recording…" : "Save"}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      ...
      {editing && (
        <Dialog open onOpenChange={() => setEditing(null)}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Correct payment {editing.receiptNo}</DialogTitle>
            </DialogHeader>
            <EditPaymentForm
              payment={editing}
              onSubmit={(f: any) => update.mutate({ id: editing.id, ...f })}
            />
          </DialogContent>
        </Dialog>
      )}
...
function PendingPaymentsTab() {
  ...
  const confirm = useMutation({
    ...
    onSuccess: () => {
      invalidate();
      toast.success("Payment confirmed and applied to invoice");
    },
    onError: (e: any) => toast.error(e.message),
  });
  const reject = useMutation({
    mutationFn: ({ id, reason }: { id: string; reason?: string }) =>
      api.fees.payments.reject(id, reason),
    onSuccess: () => {
      invalidate();
  ...
  return (
    <div className="space-y-3 pt-4">
      <p className="text-sm text-muted-foreground">
        Payments parents have submitted from the portal, awaiting confirmation. Confirming applies
        the amount to the invoice; rejecting discards it.
      </p>
      <div className="rounded-lg border bg-card">
        <Table>
          <TableHeader>
...
function EditPaymentForm({ payment, onSubmit }: { payment: Payment; onSubmit: (f: any) => void }) {
  ...
  return (
    <>
      ...
      <DialogFooter>
        <Button onClick={() => onSubmit(f)} disabled={!f.amount}>
          Save correction
        </Button>
      </DialogFooter>
    </>
  ...
  return (
    <div className="space-y-4 pt-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-7 gap-2 items-end">
          <div className="lg:col-span-2">
            ...
            <Input
              ...
              placeholder="e.g. Tuition, Transport fee…"
            />
          </div>
          <div>
          ...
          <Button
            ...
            disabled={!f.name || !f.amount || create.isPending}
          >
            <Plus className="h-4 w-4 mr-1" /> {create.isPending ? "Adding…" : "Add"}
          </Button>
        </div>
        {DUE_DATE_CATEGORIES.has(f.category) && (
          <p className="text-xs text-muted-foreground">
            Invoices billed from this item become eligible for the administrative late-payment fee
            once their due date passes.
          </p>
        )}
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <Button variant="default" onClick={() => setBillOpen(true)}>
          <Receipt className="h-4 w-4 mr-1" /> Bill a class
        </Button>
      </div>
            ...
            {visible.length === 0 ? (
              ...
              visible.map((i) => (
                <TableRow key={i.id}>
                  ...
                  <TableCell>
                    <Badge variant="outline">{categoryLabel(i.category ?? "OTHER")}</Badge>
                  </TableCell>
                  <TableCell>
                    {i.schoolClass
                      ? `${i.schoolClass.name}${i.schoolClass.stream ? " " + i.schoolClass.stream : ""}`
                      : "All"}
                  </TableCell>
                  ...
                  <TableCell>
                    <Input
                      ...
                      onBlur={(e) => {
                        ...
                        if (v !== Number(i.amount)) update.mutate({ id: i.id, amount: v });
                      }}
                    />
                  </TableCell>
                  <TableCell className="text-right">
                    <Button
                      ...
                      variant="ghost"
                      onClick={() => {
                        if (confirm("Delete this fee item?")) remove.mutate(i.id);
                      }}
                    >
                      <Trash2 className="h-4 w-4 text-destructive" />
                    </Button>
...
function BillClassDialog({ onClose, feeItems, classes, terms }: any) {
  ...
  const eligible =
    mode === "class" ? feeItems.filter((i: any) => !i.classId || i.classId === classId) : feeItems;
  ...
  async function bill() {
    ...
    try {
      const desc = eligible
        .filter((i: any) => selected[i.id])
        .map((i: any) => `${i.name}: ${money(selected[i.id])}`)
        .join(" • ");
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Bill pupils</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
                    ...
                    {filteredPupils.slice(0, 50).map((p) => (
                      ...
                      >
                        <TableCell className="w-8 py-1.5">
                          <input type="checkbox" checked={pupilIds.has(p.id)} readOnly />
                        </TableCell>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="w-40">Amount (editable)</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {eligible.length === 0 ? (
                    ...
                    eligible.map((i: any) => (
                      <TableRow key={i.id}>
                        <TableCell>
                          <input
                            ...
                            onChange={(e) => {
                              setSelected((s) => {
                                const n = { ...s };
                                if (e.target.checked) n[i.id] = Number(i.amount);
                                else delete n[i.id];
                                return n;
                              });
                            }}
                        ...
                        <TableCell>
                          <Input
                            type="number"
                            disabled={!(i.id in selected)}
                            value={selected[i.id] ?? i.amount}
                            onChange={(e) =>
                              setSelected((s) => ({ ...s, [i.id]: Number(e.target.value) }))
                            }
                            className="h-8"
                          />
                        </TableCell>
          ...
          <div className="flex justify-between text-sm border-t pt-3">
            <span className="text-muted-foreground">
              Pupils to bill: <span className="font-semibold text-foreground">{pupils.length}</span>
            </span>
            <span className="font-semibold">Each invoice total: {money(total)}</span>
          </div>
        </div>
        <DialogFooter>
          <Button onClick={bill} disabled={total <= 0 || pupils.length === 0 || busy}>
            {busy ? "Creating…" : `Create ${pupils.length} invoices`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
