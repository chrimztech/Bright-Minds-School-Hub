...
function Guardians() {
  ...
  const {
    pageItems: pagedGuardians,
    page,
    setPage,
    totalPages,
    pageSize,
    total,
  } = usePagination(filteredGuardians, 25, search);
  ...
  const pupilsByGuardian = new Map(
    pagedGuardians.map((g, i) => [g.id, pupilQueries[i]?.data ?? []]),
  );
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Linked pupil(s)</TableHead>
              ...
              {filteredGuardians.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center text-muted-foreground py-8">
                    {guardians.length === 0 ? "No parents yet." : "No parents match your search."}
                  </TableCell>
                </TableRow>
              ) : (
                pagedGuardians.map((g) => {
                  ...
                  return (
                    <TableRow key={g.id}>
                        ...
                        {linkedPupils.length === 0 ? (
                          ...
                          <div className="flex flex-wrap gap-1">
                            {linkedPupils.map((p) => (
                              <span
                                key={p.id}
                                className="inline-flex items-center rounded-md bg-primary/7 px-2 py-0.5 text-[11.5px] font-medium text-primary/80 border border-primary/10"
                              >
                                {p.fullName}
                              </span>
                            ))}
                      ...
                      <TableCell>
                        <span
                          className={`text-xs font-medium ${g.userId ? "text-green-600" : "text-muted-foreground"}`}
                        >
                      ...
                      <TableCell>
                        {canManage && (
                          <div className="flex justify-end gap-1">
                            <Button variant="ghost" size="sm" onClick={() => setLinkFor(g.id)}>
                              <Link2 className="h-4 w-4 mr-1" /> Link pupil
                            </Button>
                            <Button
                              variant="ghost"
                              size="sm"
                              disabled={!g.email}
                              onClick={() => setAccountFor(g)}
                            >
                              <KeyRound className="h-4 w-4 mr-1" />{" "}
                              {g.userId ? "Reset login" : "Create login"}
                            </Button>
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() => {
                                if (confirm(`Delete parent "${g.fullName}"?`)) remove.mutate(g.id);
                              }}
                            >
...
function GuardianForm({ onSubmit, initial }: any) {
  ...
  return (
    <DialogContent className="max-w-xl">
      <DialogHeader>
        <DialogTitle>{initial ? "Edit parent / guardian" : "New parent / guardian"}</DialogTitle>
      </DialogHeader>
        ...
        {!initial && (
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              When supplied, a parent login is created and the parent will be asked to change it.
            </p>
          </div>
        )}
        <div>
      ...
      <DialogFooter>
        <Button onClick={() => onSubmit(f)} disabled={!f.fullName}>
          Save
        </Button>
      </DialogFooter>
    </DialogContent>
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {guardian.userId ? "Reset parent login" : "Create parent login"}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Login email: <span className="font-medium text-foreground">{guardian.email}</span>
          </p>
          <div>
          ...
          <p className="text-xs text-muted-foreground">
            The parent will be required to change this password after signing in.
          </p>
        </div>
        <DialogFooter>
          ...
          <Button onClick={save} disabled={password.length < 8 || busy}>
...
function LinkPupilDialog({ guardianId, onClose }: any) {
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Link a pupil</DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter>
          <Button onClick={link} disabled={!pupilId}>
            Link
          </Button>
        </DialogFooter>
      </DialogContent>
