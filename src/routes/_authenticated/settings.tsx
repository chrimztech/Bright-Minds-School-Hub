  ...
  const save = useMutation({
    mutationFn: async () =>
      api.settings.update({
        ...
        mapUrl: f.mapUrl,
        establishedYear: f.establishedYear ? Number(f.establishedYear) : undefined,
        registrationNo: f.registrationNo,
        tpin: f.tpin,
        headTeacher: f.headTeacher,
        headTeacherSignatureUrl: f.headTeacherSignatureUrl,
        deputyHead: f.deputyHead,
      }),
    onSuccess: () => {
      toast.success("Saved");
        ...
        <Card>
          <CardContent className="p-6 space-y-3">
            ...
            <div>
              ...
              <ImageUploadField
                value={f.logoUrl ?? ""}
                onChange={(url) => setF({ ...f, logoUrl: url })}
            ...
            <div>
              ...
              <ImageUploadField
                value={f.bannerUrl ?? ""}
                onChange={(url) => setF({ ...f, bannerUrl: url })}
                placeholder="https://…/school-photo.jpg"
              />
              <p className="text-xs text-muted-foreground mt-1">
                Shown on everyone's dashboard landing page when set.
              </p>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <Input
                  value={f.headTeacher ?? ""}
                  onChange={(e) => setF({ ...f, headTeacher: e.target.value })}
                />
              </div>
              <div>
                ...
                <Input
                  value={f.deputyHead ?? ""}
                  onChange={(e) => setF({ ...f, deputyHead: e.target.value })}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setF({ ...f, establishedYear: e.target.value })}
                />
              </div>
            </div>
            <div>
              ...
              <ImageUploadField
                value={f.headTeacherSignatureUrl ?? ""}
                onChange={(url) => setF({ ...f, headTeacherSignatureUrl: url })}
                placeholder="https://… (image of their signature)"
              />
              <p className="text-xs text-muted-foreground mt-1">
                Stamped on report cards alongside the class teacher's signature.
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input
                  value={f.tpin ?? ""}
                  onChange={(e) => setF({ ...f, tpin: e.target.value })}
                />
              </div>
            </div>
            <div className="border-t pt-3 mt-2">
              ...
              <div className="grid grid-cols-2 gap-3 mt-2">
                <div>
                  ...
                  <Input
                    value={f.plotNumber ?? ""}
                    onChange={(e) => setF({ ...f, plotNumber: e.target.value })}
                  />
                </div>
                <div>
                  <Label>P.O. Box</Label>
                  <Input
                    value={f.poBox ?? ""}
                    onChange={(e) => setF({ ...f, poBox: e.target.value })}
                  />
                </div>
              </div>
              ...
              {mapSrc && (
                <div className="mt-3">
                  ...
                  <a
                    href={
                      f.mapUrl ||
                      `https://www.openstreetmap.org/?mlat=${f.latitude}&mlon=${f.longitude}#map=17/${f.latitude}/${f.longitude}`
                    }
                    target="_blank"
                    rel="noreferrer"
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input
                  value={f.facebookUrl ?? ""}
                  onChange={(e) => setF({ ...f, facebookUrl: e.target.value })}
