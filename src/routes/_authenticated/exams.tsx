    ...
    <>
          ...
          canManage ? (
            ...
            >
              ...
              <DialogContent>
                ...
                <DialogFooter>
                  <Button onClick={() => create.mutate()} disabled={!f.name}>
                    Save
                  </Button>
                </DialogFooter>
              </DialogContent>
        ...
        <div className="flex items-center gap-3">
          ...
          <Select value={termFilter} onValueChange={setTermFilter}>
            <SelectTrigger className="w-72">
              <SelectValue placeholder="All terms / years" />
            </SelectTrigger>
            <SelectContent>
          ...
          {termFilter && (
            <Button variant="ghost" size="sm" onClick={() => setTermFilter("")}>
              Clear
            </Button>
          )}
          <span className="text-sm text-muted-foreground ml-auto">
            {data.length} exam{data.length !== 1 ? "s" : ""}
          </span>
        </div>
              ...
              {data.length === 0 ? (
                ...
                data.map((e) => (
                  <TableRow key={e.id}>
                    ...
                    <TableCell>
                      {e.assessmentType ? (
                        <Badge variant="outline">
                          {ASSESSMENT_LABELS[e.assessmentType] ?? e.assessmentType}
                        </Badge>
                      ) : (
                    ...
                    <TableCell>
                      {e.termId ? (
                        <Badge variant="secondary">{termLabel(e.termId)}</Badge>
                      ) : (
                        <span className="text-muted-foreground text-xs">—</span>
                      )}
                    </TableCell>
                      ...
                      {canManage && (
                        <>
                          ...
                          <Button
                            ...
                            size="icon"
                            onClick={() => {
                              if (confirm(`Delete ${e.name}?`)) remove.mutate(e.id);
                            }}
                          >
      ...
      >
        <DialogContent>
          ...
          <ExamFormFields f={f} setF={setF} terms={terms} />
          <DialogFooter>
            <Button onClick={() => update.mutate()} disabled={!f.name || update.isPending}>
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <Select
            value={f.assessmentType}
            onValueChange={(v) => setF({ ...f, assessmentType: v as AssessmentType })}
          >
            <SelectTrigger>
            ...
            <SelectContent>
              ...
              <SelectItem value="TEST_1">Test 1</SelectItem>
              <SelectItem value="TEST_2">Test 2</SelectItem>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div>
          ...
          <Input
            type="date"
            value={f.examDate}
            onChange={(e) => setF({ ...f, examDate: e.target.value })}
          />
        </div>
        <div>
          <Label>Out of</Label>
          <Input
            type="number"
            value={f.outOf}
            onChange={(e) => setF({ ...f, outOf: Number(e.target.value) })}
          />
        </div>
        <div>
          <Label>Weight</Label>
