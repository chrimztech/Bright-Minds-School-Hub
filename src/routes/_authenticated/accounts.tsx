...
function Summary() {
  ...
  const { data: canteenSales = [] } = useQuery({
    queryKey: ["canteen-sales-accounts"],
  ...
  const { data: admissions = [] } = useQuery({
    queryKey: ["admissions-accounts"],
    queryFn: () => api.admissions.list(),
  });
  ...
  const income =
    payments.filter((p) => p.status === "CONFIRMED").reduce((a, r) => a + Number(r.amount), 0) +
    ...
    <div className="grid gap-3 sm:grid-cols-3">
      <Card>
        <CardContent className="p-4">
          ...
          <p className="text-2xl font-semibold text-emerald-600">{money(income)}</p>
        </CardContent>
      </Card>
      <Card>
        <CardContent className="p-4">
          <p className="text-xs text-muted-foreground">Total expenses</p>
          <p className="text-2xl font-semibold text-rose-600">{money(expense)}</p>
        </CardContent>
      </Card>
      <Card>
        <CardContent className="p-4">
          <p className="text-xs text-muted-foreground">Net</p>
          <p className="text-2xl font-semibold">{money(income - expense)}</p>
        </CardContent>
      </Card>
    </div>
  );
                      ...
                      <>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (
                              confirm(
                                `Reverse this expense of ${money(e.amount)}? It will be removed.`,
                              )
                            )
                              remove.mutate(e.id);
                          }}
...
function ExpenseForm({ onSubmit, initial }: any) {
  ...
  return (
    <DialogContent>
      <DialogHeader>
        <DialogTitle>{initial ? "Correct expense" : "Record expense"}</DialogTitle>
      </DialogHeader>
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Input
              type="date"
              value={f.spentOn}
              onChange={(e) => setF({ ...f, spentOn: e.target.value })}
            />
          </div>
          <div>
            <Label>Amount</Label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label>Payee</Label>
            <Input value={f.payee} onChange={(e) => setF({ ...f, payee: e.target.value })} />
          </div>
          <div>
            <Label>Ref no.</Label>
            <Input value={f.refNo} onChange={(e) => setF({ ...f, refNo: e.target.value })} />
          </div>
        </div>
        <div>
...
function Income() {
  ...
  const feeRows: IncomeRow[] = data
    .filter((p) => p.status === "CONFIRMED")
    .map((p) => ({
      id: p.id,
      paidOn: p.paidOn,
      pupilName: p.pupil?.fullName ?? "—",
      classId: p.pupil?.schoolClass?.id,
      className: p.pupil?.schoolClass?.name,
      category: incomeCategory(p),
      method: p.method,
      amount: Number(p.amount),
    }));
  const canteenRows: IncomeRow[] =
    !termId && !academicYearId
      ? canteenSales
          .filter(
            (s) =>
              (!classId || s.pupil?.schoolClass?.id === classId) &&
              (!pupilId || s.pupilId === pupilId || s.pupil?.id === pupilId),
          )
          .map((s) => ({
            id: s.id,
            paidOn: s.servedOn,
            pupilName: s.pupil?.fullName ?? "Walk-in",
            classId: s.pupil?.schoolClass?.id,
            className: s.pupil?.schoolClass?.name,
            category: "FOOD",
            method: s.paymentMethod ?? "cash",
            amount: Number(s.total),
          }))
      : [];
  const admissionRows: IncomeRow[] =
    !termId && !academicYearId
      ? admissions
          .filter((a) => Number(a.regFeePaid ?? 0) > 0)
          .filter(
            (a) =>
              (!classId || a.pupil?.schoolClass?.id === classId) &&
              (!pupilId || a.pupil?.id === pupilId),
          )
          .map((a) => ({
            id: a.id,
            paidOn: a.regFeePaidOn ?? a.interviewDate ?? "—",
            pupilName: a.fullName + (a.pupil ? "" : " (applicant)"),
            classId: a.pupil?.schoolClass?.id,
            className: a.pupil?.schoolClass?.name ?? a.targetClass?.name,
            category: "REGISTRATION",
            method: a.regFeePaymentMethod ?? "cash",
            amount: Number(a.regFeePaid),
          }))
      : [];
  const confirmed = [...feeRows, ...canteenRows, ...admissionRows].sort((a, b) =>
  ...
  const breakdown = INCOME_FILTERS.filter((f) => f.value !== "ALL")
    .map((f) => ({
      ...f,
    ...
    <div className="mt-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <span className="ml-auto text-sm text-muted-foreground self-center">
          {visible.length} payment{visible.length !== 1 ? "s" : ""} · {money(total)}
        </span>
      </div>
