                            ...
                            <div>
                              ...
                              <p className="font-semibold">
                                {details
                                  ...
                                  : childAtt.length ? `${childPct}% (${childPresent}/${childAtt.length})` : "—"}
                              </p>
                            </div>
                            <div>
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-xl border bg-muted/20 p-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div><p className="text-xs text-muted-foreground">Term attendance</p><p className="font-semibold text-lg">{report.attendance.percentage.toFixed(0)}%</p><p className="text-xs text-muted-foreground">{report.attendance.present}/{report.attendance.total} · {report.attendance.present} present · {report.attendance.absent} absent · {report.attendance.late} late</p></div>
      </div>
      {teacher && (
  ...
  return (
    <div className="p-10 font-sans text-black text-sm">
      ...
      <div className="border border-gray-300 rounded p-3 grid grid-cols-4 gap-3 text-center mb-5">
        ...
        <div><p className="text-xs text-gray-500">Attendance rate</p><p className="font-bold">{att.percentage.toFixed(0)}%<span className="font-normal text-gray-500 text-xs"> ({att.present}/{att.total})</span></p></div>
      </div>
