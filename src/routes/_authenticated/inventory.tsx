...
import { Plus, AlertTriangle, ArrowUpDown, Trash2, Pencil } from "lucide-react";
...
const CATS = ["Stationery","Textbooks","Exercise books","Uniform","P.E. Attire","Jersey","Socks","Cleaning","Sports","Furniture","Computers","Kitchen","Medical","Office","Other"];
...
function InventoryPage() {
  ...
  const { pageItems: pagedInv, page, setPage, totalPages, pageSize, total } = usePagination(data, 25);
  ...
  const update = useMutation({
    mutationFn: (f: any) => api.inventory.items.update(editing.id, f),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["inv"] }); toast.success("Item updated"); setEditing(null); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  return (
    <>
      ...
      <div className="p-6"><div className="rounded-lg border bg-card"><Table>
        ...
        <TableBody>
          {data.length === 0 ? <TableRow><TableCell colSpan={6}><EmptyState /></TableCell></TableRow> : pagedInv.map((i) => (
            <TableRow key={i.id}>
              ...
              <TableCell className="flex gap-1 justify-end">
                ...
                {canManage && <Button variant="ghost" size="icon" onClick={() => setEditing(i)}><Pencil className="h-4 w-4" /></Button>}
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        {editing && <ItemForm initial={editing} title="Edit item" hideQuantity onSubmit={(f: any) => update.mutate(f)} />}
      </Dialog>
    </>
  );
}

function ItemForm({ initial, title, hideQuantity, onSubmit }: any) {
  const [f, setF] = useState(initial
    ? { name: initial.name, category: initial.category ?? "Stationery", sku: initial.sku ?? "", unit: initial.unit ?? "pcs", quantity: initial.quantity, reorderLevel: initial.reorderLevel ?? 0, unitCost: initial.unitCost ?? 0, location: initial.location ?? "" }
    : { name: "", category: "Stationery", sku: "", unit: "pcs", quantity: 0, reorderLevel: 0, unitCost: 0, location: "" });
  return (
    <DialogContent><DialogHeader><DialogTitle>{title ?? "New item"}</DialogTitle></DialogHeader>
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          {!hideQuantity && <div><Label>Opening qty</Label><Input type="number" value={f.quantity} onChange={(e) => setF({ ...f, quantity: Number(e.target.value) })} /></div>}
        ...
        {hideQuantity && <p className="text-xs text-muted-foreground">Quantity is changed via "Move" (stock in/out), not here — that keeps a record of why it changed.</p>}
      </div>
